...
static esp_err_t read_data(esp_lcd_touch_handle_t tp)
{
    ...
    /* CST820 requires this wake/ack sequence. Keep it in the same order as
     * Waveshare's working driver: acknowledge after reading coordinates, not
     * before. */
    uint8_t wake = 0x01;
    touch_cst820_i2c_write(tp, DATA_START_REG, &wake, 1);
